...
public class GUI extends Application implements Runnable {
    ...
    private static TextField ipBox;
    private static TextField portBox;
    private static TextArea chatBox;
    private static VBox usersBox;
    private static boolean is_left = false;
    ...
    protected void updateGUIUsers() {
        ...
        Platform.runLater(() -> {
            ...
            usersBox.getChildren().clear(); // Use clear() to remove all existing nodes
            Label Users = new Label("Users");
            Users.setStyle("-fx-font-size: 20px; -fx-font-weight: bold; -fx-text-fill: white; -fx-font-family: 'Arial'");
            Label activePeople = new Label("Active People: ");
            activePeople.setStyle("-fx-font-size: 15px; -fx-text-fill: white; -fx-font-family: 'Arial'");
            ...
            if (data.users != null && !data.users.isEmpty()) {
                String[] userList = data.users.split(",");
                System.out.println("THE USER LIST IS" + Arrays.toString(userList)+ "Data.users is "+data.users);
                String[] userList_no_dupe = Arrays.stream(userList).distinct().toArray(String[]::new);// Assuming '#' is your separator
                for (String user : userList_no_dupe) {
...
//        chatBox.setPrefWidth(300);
//
//        System.out.println("Chat box init");
//
//        ScrollPane scrollPane = new ScrollPane(chatBox);
//        scrollPane.setStyle("-fx-background-color: transparent; -fx-background: #5B9279; -fx-text-fill: white; -fx-font-family: 'Arial'");
//        scrollPane.setFitToHeight(true);
...
//            data.outMessages = message;
//            if (!message.isEmpty()) {
//                chatBox.appendText(data.outMessages);
//                // log message
//                sendBox.clear();
//            }
//        });
